...
const ROOT = '/Users/saieeshwar/Projects/Work/work/napkin/ADF/clan/test-sandbox/happy/lite'
...
function h1ClanPrompt(hop) {
  ...
  return `You are the CLAN H1 revision agent for hop ${hop.n} (role: ${hop.role}). The clan repo root is /Users/saieeshwar/Projects/Work/work/napkin/ADF/clan; the 'clan' binary (v1.1.0) is on PATH. All paths below are absolute.
...
function h1AdhocPrompt(hop) {
  ...
  return `You are the AD-HOC H1 revision agent for hop ${hop.n} (role: ${hop.role}). All paths below are absolute.
...
function h2ClanPrompt(idx) {
  ...
  const n = idx + 1
  return `You are the CLAN H2 chain agent, hop ${n}, role ${role}. The 'clan' binary (v1.1.0) is on PATH. All paths below are absolute.
...
function h2AdhocPrompt(idx) {
  ...
  const n = idx + 1
  return `You are the AD-HOC H2 chain agent, hop ${n}, role ${role}. All paths below are absolute.
...
function h2LeadClanPrompt() {
  const dir = `${ROOT}/h2-clan`
  return `You are the CLAN H2 LEAD PARTNER (final hop 10). The 'clan' binary (v1.1.0) is on PATH. All paths below are absolute.
...
function h2LeadAdhocPrompt() {
  const dir = `${ROOT}/h2-adhoc`
  return `You are the AD-HOC H2 LEAD PARTNER (final hop 10). All paths below are absolute.
...
const verdict = await agent(
  `You are the H1 fidelity verifier. The 'clan' binary (v1.1.0) is on PATH. All paths below are absolute. Compare both H1 final artifacts against the 8 required changes.
