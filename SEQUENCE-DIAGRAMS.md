# XON Sequence Diagrams

All diagrams use Mermaid syntax. Render with any Mermaid-compatible viewer (GitHub, Obsidian, Mermaid Live Editor).

---

## 1. First XON Creation (Human-Initiated)

A human opens the app, clicks "New XON", enters a title and brief. The SDK creates the container and a first-pass agent produces the initial document.

```mermaid
sequenceDiagram
    actor Human
    participant App as XON App (Tauri)
    participant SDK as XON SDK (Rust)
    participant FS as File System
    participant Agent as First Agent (LLM)

    Human->>App: Click "New XON"
    App->>Human: Show creation dialog (title + brief)
    Human->>App: "Invoice Review" + "Analyze Q2 invoice from Acme Corp"

    App->>SDK: xon.create(title, brief, type="invoice")

    SDK->>SDK: Select template (invoice)
    SDK->>SDK: Generate UUID for document
    SDK->>SDK: Write manifest.yaml (xon_version, id, title, created_at)
    SDK->>SDK: Copy spec/xon.md and spec/agent-guide.md from template
    ...
    SDK->>SDK: Package into invoice-review.xon (ZIP)

    SDK->>FS: Write invoice-review.xon
    SDK-->>App: XON file path

    App->>App: Open XON — show pending state in document WebView
    App->>Agent: SDK assembles context (guide + context.md + schema)

    Agent->>Agent: Read agent_guide (understand XON format)
    ...
    SDK->>SDK: Repackage into invoice-review.xon

    SDK-->>App: Updated XON ready
...
How the SDK prepares context for an agent receiving an existing XON file.

```mermaid
sequenceDiagram
    participant Pipeline as Orchestrator
    participant SDK as XON SDK (Rust)
    participant XON as .xon file (ZIP)
    participant Agent as Agent (LLM)
    participant Cache as Session Cache

    Pipeline->>SDK: sdk.open("invoice-review.xon", agent_role="validator")

    SDK->>XON: Open ZIP archive
    SDK->>XON: Read manifest.yaml (by_name — first entry)
    SDK->>XON: Read spec/agent-guide.md
    SDK->>XON: Read agent/context.md
    SDK->>XON: Read agent/output-schema.json
    SDK->>XON: Read shared/data.yaml
    SDK->>XON: Read agent/decision-chain.yaml
    SDK->>XON: Read human/patches.yaml (optional)
    SDK->>XON: Close ZIP (no full extraction)
...
## 3. Agent Output and XON Packaging

How the SDK validates and packages agent output into a new XON file.

```mermaid
sequenceDiagram
    participant Agent as Agent (LLM)
    participant SDK as XON SDK (Rust)
    participant XON as New .xon file
    ...
    SDK->>SDK: Generate new UUID for this XON version

    SDK->>XON: Package all files into new .xon (ZIP + DEFLATE)

    SDK-->>Pipeline: New .xon file path
```
...
sequenceDiagram
    ...
    Rust->>Doc: Serve updated xon://current/edit-bridge.js (edit mode active)
    ...
    Doc->>Shell: postMessage({ type: "xon-edit", id: "heading-0", content: "Updated Invoice Review — Amended" })

    Shell->>Shell: Validate message type ("xon-edit" allowed)
    Shell->>Shell: Validate id (exists in current data-adf-id map)
    Shell->>Rust: invoke("save_patch", { id: "heading-0", content: "..." })

    Rust->>FS: Open .xon ZIP
...
A complete pipeline showing how XON files flow through multiple agents, accumulating decisions and lineage.

```mermaid
sequenceDiagram
    actor Human
    participant App as XON App
    participant SDK as XON SDK
    participant A1 as Extractor Agent
    participant A2 as Validator Agent
    participant A3 as Approval Agent

    Human->>App: New XON — "Process Acme Q2 Invoice"
    App->>SDK: create(brief, type="invoice")
    SDK-->>App: invoice-v0.xon (pending state, no data)

    App->>SDK: open(invoice-v0.xon, agent=extractor)
    SDK-->>A1: Context (guide + context.md + schema + empty data)
    A1-->>SDK: { mode: "full-html", structured: {vendor, total, ...}, human: {html, css} }
    SDK->>SDK: Package → invoice-v1.xon (lineage: parent=v0)
    note right of SDK: decision-chain: [extractor: extracted 12 fields]

    SDK-->>App: invoice-v1.xon ready
    App->>App: Render — human sees formatted invoice

    App->>SDK: open(invoice-v1.xon, agent=validator)
    ...
    SDK->>SDK: Package → invoice-v2.xon (lineage: parent=v1)
    note right of SDK: decision-chain: [extractor, validator]

    SDK-->>App: invoice-v2.xon ready
    ...
    App->>SDK: open(invoice-v2.xon, agent=approver)
    ...
    SDK->>SDK: Package → invoice-v3.xon (lineage: parent=v2)
    note right of SDK: decision-chain: [extractor, validator, approver]

    SDK-->>App: invoice-v3.xon ready
...
How an agent with no SDK access can still produce valid XON output.

```mermaid
sequenceDiagram
    participant Orch as Orchestrator
    participant SDK as XON SDK
    participant XON as .xon file
    participant API as LLM API (no SDK)
    participant Agent as External Agent

    Orch->>SDK: sdk.export_static("invoice.xon")
    SDK->>XON: Read all sections
    SDK->>SDK: Assemble static export:
    note right of SDK: {<br/>  xon_version: "1.0",<br/>  agent_guide: "...spec/agent-guide.md...",<br/>  task: "...agent/context.md...",<br/>  output_schema: {...},<br/>  shared_data: {...},<br/>  decision_history: [...]<br/>}
    SDK-->>Orch: invoice-static.json

    Orch->>API: POST /chat/completions
    note right of Orch: System: "You are processing a XON document.<br/>Read agent_guide first."<br/>User: [invoice-static.json contents]
    ...
    Orch->>SDK: sdk.package_output(response_json, source_xon="invoice.xon")
    SDK->>SDK: Validate against output-schema.json
    SDK->>SDK: Package into new invoice-v2.xon
    SDK-->>Orch: invoice-v2.xon (valid XON, full lineage preserved)

    note over Agent: Agent never knew it was working with XON.
    note over Orch: Output is a fully valid XON file.
```
...
How the Tauri app renders a XON file in the document WebView.
...
sequenceDiagram
    ...
    participant Rust as Tauri Rust Backend
    participant Shell as Shell WebView (Svelte)
    participant Protocol as xon:// Protocol Handler
    participant Doc as Document WebView (sandboxed)
    ...
    Doc->>Protocol: GET xon://current/styles.css
    Protocol->>Rust: Serve human/styles.css from ZIP
    Rust-->>Doc: CSS (scoped)

    Doc->>Protocol: GET xon://current/bindings.js
    Protocol->>Rust: Serve generated bindings.js
    Rust-->>Doc: window.__XON__ = { data: {...} }

    Doc->>Protocol: GET xon://current/assets/chart.svg
...
## 8. XON File Creation — CLI Path

How a developer creates a XON from the command line.

```mermaid
sequenceDiagram
    actor Dev as Developer
    participant CLI as xon CLI
    participant SDK as XON SDK (Rust)
    participant FS as File System

    Dev->>CLI: xon create --type invoice --title "Q2 Invoice"
    ...
    SDK->>SDK: Copy spec/xon.md (pinned to current SDK version)
    ...
    SDK->>SDK: Package into q2-invoice.xon

    SDK->>FS: Write q2-invoice.xon
    SDK-->>CLI: Created: q2-invoice.xon

    CLI-->>Dev: ✓ q2-invoice.xon created
    Dev->>CLI: xon read q2-invoice.xon agent
    CLI->>SDK: sdk.read("q2-invoice.xon", section="agent")
    SDK-->>CLI: YAML output of all agent/ files
    CLI-->>Dev: Displays agent context (for debugging)

    Dev->>CLI: xon validate q2-invoice.xon
    CLI->>SDK: sdk.validate("q2-invoice.xon")
