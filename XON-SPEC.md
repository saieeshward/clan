# XON Specification v1.0

**eXchange Object Notation**
Pronounced "axon"
...
XON (eXchange Object Notation) is an open container format for passing structured context between AI agents and rendering that context for humans. A `.xon` file is simultaneously:
...
XON is inspired by the axon: the nerve fibre that carries signals between neurons. Agents are the neurons. XON files are the axons carrying context between them.
...
### Sufficiency
Every XON file carries its own specification (`spec/`). An agent that has never encountered XON before can understand the format and produce valid output from a single file.

### Openness
The format specification and reference SDK are open (Apache 2.0). No XON-compatible implementation requires a licence. The format does not depend on any proprietary system.
...
Every valid XON archive MUST contain these files:
...
The manifest is the root index of every XON file. It MUST be valid YAML and MUST be the first entry in the ZIP.

### Schema

```yaml
# Required fields
xon_version: 1                          # integer — major version
xon_version_minor: 0                    # integer — minor version
...
# Optional: lineage (absent on first XON in a chain)
lineage:
  parent_id: "3f9a2b1c-..."            # UUID of parent XON
  parent_uri: "file:///docs/inv-001.xon"  # URI to parent file
...
- Injected into the human rendering as `window.__XON__.data` by the app
...
Instructions for the agent that will process the next XON in the lineage chain. Written by the current agent or a human orchestrator.

```markdown
# For the Validation Agent

You will receive a XON document with extracted invoice fields.
...
- SHOULD use the app's CSS design tokens (`var(--xon-accent)`, etc.)
...
The app resolves `{{token}}` expressions by reading `shared/data.yaml` and injecting `window.__XON__ = { data: { ... } }` before rendering. The HTML references tokens using the double-brace syntax. Complex expressions are not supported — only direct key references and dot-notation for nested keys (`{{line_items.0.amount}}`).
...
5. Serve via `xon://` custom protocol to document WebView
...
The complete XON specification. This file is the canonical spec that travels inside every `.xon` file. Its contents are identical to the public specification. Version pinned at creation time.

Purpose: enables any consumer — agent, human, tooling — to understand the XON format from a single file, without external documentation.
...
Use for: document-generating agents, report writers, first-time XON creation. Any agent with full design responsibility.
...
1. User opens XON in the app — human view rendered in sandboxed WebView
2. User activates edit mode (toolbar button or keyboard shortcut)
3. App injects edit bridge into document WebView via `xon://current/edit-bridge.js`
4. Edit bridge enables `contenteditable` on elements with `data-adf-id`
5. On text blur, edit bridge sends `postMessage({ type: "xon-edit", id, content })`
6. Shell WebView receives postMessage, calls Tauri `invoke("save_patch", { id, content })`
7. Rust backend appends to `human/patches.yaml` in the extracted XON
...
Every XON is a node in a directed acyclic graph (DAG) of document versions. The `lineage` block in `manifest.yaml` records the parent.
...
lineage:
  ...
  parent_uri: "file:///invoices/inv-001.xon"
  delta: "Validation complete. Vendor name corrected from 'ACME' to 'Acme Corporation'."
```

- `parent_id` — UUID of the parent XON (from its `manifest.yaml`)
...
- First XON in a chain has no `lineage` block (or `lineage: null`)
- A XON MUST NOT reference itself as its own parent
- The lineage chain is reconstructed by following `parent_uri` references
- Parent XON files are never embedded — only referenced
...
Large context data (full reasoning traces, embedding indexes, raw conversation logs) lives outside the XON file and is referenced by URI with a content hash for integrity.
...
external:
  - id: "context-store"
    ...
    access: "read"
  - id: "vector-index"
    uri: "pinecone://xon-contexts/invoice-cluster"
    type: "vector-store"
    description: "Semantic search index for related XON files"
    access: "read"
```
...
The `content-hash` enables any consumer to verify the external content hasn't changed since the XON was written. Consumers MAY skip verification but SHOULD warn if verification fails.

---

## 14. Agent Injection Protocol

When the SDK loads a XON file for an agent, it assembles a context object in this order:
...
XON (eXchange Object Notation) is a file format for passing context
between AI agents and rendering it for humans. You received a .xon file.
...
The SDK packages your output into a valid .xon file automatically.
...
```python
xon_sdk.export_static("document.xon", output="document-static.json")
```
...
The agent reads this, follows the embedded `agent_guide`, produces JSON matching `output_schema`, and returns it. The SDK receives the response and packages it into a valid `.xon` file. **The agent never knew it was working with XON.**
...
- **Document WebView** — sandboxed. No Tauri IPC. Renders agent-generated HTML via `xon://` custom protocol.
...
The `xon://` custom protocol is served by Rust from the in-memory extracted ZIP. No filesystem access from within the document WebView.

### Edit Bridge Security

The postMessage bridge between document WebView and shell WebView MUST:
- Validate message `type` against an allowlist (`xon-edit` only)
...
A XON file is **valid** if and only if:

- [ ] It is a readable ZIP archive
- [ ] `manifest.yaml` exists and parses as valid YAML
- [ ] `manifest.yaml` contains all required fields (`xon_version`, `xon_version_minor`, `id`, `title`, `created_at`, `updated_at`)
- [ ] `manifest.yaml` `id` is a valid UUID v4
- [ ] All files listed in `manifest.yaml` `files[]` exist in the archive
- [ ] `spec/xon.md` exists and is non-empty
...
A XON file is **content-valid** if additionally:
...
- Readers MUST reject files where `xon_version` > their supported major version
- Readers MUST accept files where `xon_version_minor` > their supported minor version (forward compatible)
...
XON uses a two-integer version scheme declared in `manifest.yaml`:

```yaml
xon_version: 1        # major — breaking changes increment this
xon_version_minor: 0  # minor — backwards-compatible additions increment this
```
...
XON is designed to be compatible with the Model Context Protocol (MCP). The agent context object assembled by the SDK (Section 14) follows MCP resource conventions.

An XON SDK MAY expose XON files as MCP resources:

```json
{
  "uri": "xon://document/shared/data",
...
An XON file may reference MCP resources in its `external` block (type `mcp-resource`). The SDK resolves these references via the MCP client when assembling agent context.
...
**App (required for XON viewer implementation)**
- [ ] File association registration (`.xon` → app)
- [ ] Cold-open file path buffering (wait for WebView ready signal)
- [ ] Multi-webview architecture (shell + sandboxed document)
- [ ] `xon://` custom protocol handler serving from in-memory ZIP
