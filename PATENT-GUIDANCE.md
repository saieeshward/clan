# CLAN Patent Strategy

---

## The Short Answer

**Do not patent the CLAN format.** Patent the app features.
...
|---|---|---|---|
| CLAN format spec | Defensive publication (arXiv) | Free | Days |
| CLAN format spec | Apache 2.0 patent grant | Free (in licence) | Immediate |
...
4. Submit a paper titled something like:
   > "CLAN: eXchange Object Notation — An Open Container Format for Multi-Agent AI Context Exchange"
...
By releasing the spec and SDK under Apache 2.0, every contributor automatically grants all users a royalty-free, perpetual licence to any patents they hold that are necessary to implement CLAN. This means:

- If you later obtain patents on CLAN concepts, you've already granted free use to all implementors
...
If you want patent protection for specific features of the CLAN App (not the format spec), a **provisional patent application** at the USPTO gives you 12 months of "patent pending" status at low cost.
...
Do NOT try to patent the CLAN format itself — prior art (your arXiv publication) prevents this, and even if granted, it would harm adoption.
...
Register "CLAN" as a trademark for software and file format categories. This protects the brand even if you cannot patent the format.

Anyone can implement the CLAN format (open spec), but they cannot call their product "CLAN" without a trademark licence.
...
| File USPTO trademark (CLAN) | Within 1 month | ~$250 | Brand protection |
