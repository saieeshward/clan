# XON Licensing Strategy

---

## Recommendation

Use a **three-tier licensing model** — different licences for different layers of the XON ecosystem. This is the model used by successful open formats that maintain commercial products alongside them (PDF/Adobe, Kubernetes/Google, Terraform/HashiCorp pre-2023).

| Layer | Licence | Rationale |
|---|---|---|
| XON Format Specification | Apache 2.0 | Open, patent-protected, commercial-friendly |
| XON SDK (Rust core + bindings) | Apache 2.0 | Same — consistent with spec |
| XON App (Tauri viewer) | Proprietary | Commercial moat — best viewer, not exclusive |
...
Apache 2.0 includes an explicit **patent grant**. Every contributor to the spec or SDK automatically grants all users a royalty-free licence to any patents they hold that are necessary to implement XON. This means:

- Teams adopting XON have legal clarity — no surprise patent claims later
- Researchers and academics can publish XON-based work freely
- Enterprises can build XON-compatible products without legal review headaches
- This is the same reason Kubernetes, TensorFlow, Android, and Swift all use Apache 2.0

### Commercial use allowed

Apache 2.0 allows commercial use. Companies can build XON-compatible products and charge for them. This drives adoption — if organisations cannot commercialise around XON, they will not invest in adopting it.

### Attribution preserved

Apache 2.0 requires preserving copyright notices and attributing the original authors. XON spec implementations must acknowledge the origin. This builds brand recognition as adoption grows.

### Why not MIT?

MIT has no patent grant. A competitor could implement XON, patent aspects of their implementation, and assert those patents against the XON community. Apache 2.0 closes this risk.
...
The XON format being open does not require the best viewer to be open. This is the standard pattern for successful open formats:
...
The proprietary app wins on quality, not exclusivity. Anyone can build a XON viewer — and some will. The goal is to build the one people choose because it is the best, not because it is the only option.
...
```markdown
## Licence
XON Specification and Reference SDK — Apache License 2.0
XON App — Proprietary (© [Your Organisation Name])
```
...
### The .xon Files Themselves

XON files that ship as part of the spec (example files, templates) should be explicitly dedicated to the public domain or CC0 — so anyone can use them as a starting point without attribution obligations.
...
**1. Trademark**
Register "XON" and the XON logo as trademarks. Anyone can implement the format, but they cannot call their implementation "XON" without a trademark licence. This is how Red Hat operates around Linux — they can't stop people from using Linux, but "Red Hat Enterprise Linux" is theirs.
