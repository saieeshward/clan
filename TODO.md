...
- [ ] **(follow-up) C-CONF T20 / F2b is unexpectedly GREEN** — the conformance harness shows T20 (no stale-view hint on a bound-key patch) passing on the current binary, though F2b is still listed open below. Confirm whether F2b actually landed or T20's assertion is too weak before flipping the claim. (the earlier conformance reds were a harness path bug — binary must be passed as an absolute path; cwd is a temp dir.)
