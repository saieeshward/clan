...
- [ ] **Double-save on every edit** — `app/src-tauri/src/main.rs:441–448` [TEST]
...
- [ ] **Double file read in `open_clan`** — `app/src-tauri/src/main.rs:72–96` [TEST]
  `ClanFile::open()` reads the file; then `std::fs::read(&p)` reads it again. Use `clan.raw_bytes()` instead of the second read.

- [ ] **`has_entry` fully decompresses the entry just to check existence** — `crates/clan-sdk/src/container.rs:76–78` [TEST]
  `read_named` allocates and decompresses into `Vec<u8>`. Should check the ZIP central directory only, without decompressing.

- [ ] **O(n) ZIP opens in `pack()`** — `crates/clan-sdk/src/pack.rs:261–273` [TEST]
...
- [ ] **`strip_scripts` calls `to_lowercase()` on every loop iteration** — `crates/clan-sdk/src/pack.rs:483–492` [OPT]
...
- [ ] **`patch_decision` runs full schema validation just to append a decision** — `crates/clan-sdk/src/pack.rs:594–615` [TEST]
...
- [ ] **`pack_html` with `context_handoff` clones archive bytes 3× ** — `crates/clan-sdk/src/pack.rs:381–394` [TEST] [IMP]
...
- [ ] **`strip_on_handlers` mishandles `>` inside non-`on*` attribute values** — `crates/clan-sdk/src/pack.rs:499–546` [TEST][OPT]
...
- [ ] **`cmd_agent_help` hardcodes version `v0.13`** — `crates/clan-cli/src/main.rs:497` [GOOD]
  Binary is v1.0.0. Replace with `env!("CARGO_PKG_VERSION")`.

- [ ] **`ammonia` is a dead dependency** — `crates/clan-sdk/Cargo.toml` [GOOD]
...
- [ ] **`auto_inject_adf_ids` makes 10 full passes over the HTML** — `app/src-tauri/src/main.rs:333–340` [GOOD]
  One pass per tag type (`h1…h6, p, li, td, th`). Rewrite as a single-pass state machine.

- [ ] **`patch-html` silent failure on non-matching selector** — `crates/clan-sdk/src/pack.rs:449–475` [GOOD][DEV]
...
- [ ] **`clan create` uses a positional arg while `pack`/`pack-html` use `--output`** — `crates/clan-cli/src/main.rs:248` [TEST][OPT]
...
- [ ] **Schema injection is raw JSON** — `crates/clan-sdk/src/inject.rs:59` [TEST HEAVY]
...
- [ ] **Agent guide injected in full on every `clan read agent`** — `crates/clan-sdk/src/inject.rs:51` [OPT][HEAVY TEST]
...
- [ ] **`fields_changed` on old decision-chain entries is noise** [HEAVY TEST]
