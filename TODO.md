...
### Benchmark-driven product fixes (2026-06-11, post-heavy)

- [x] **Columnar TOON (C-TOON 1.8% → 57.1%)** — `to_toon` repeated every field name on every row, so savings were marginal. Added columnar encoding for uniform scalar-object arrays: `key [N] @col1 col2 …` header + one value row per object (toon.rs). Decoder (`parse_toon`) inverts it; falls back to the row form when keys differ, a value is a nested collection, or any value/key contains a space/newline (keeps the lossless round-trip the schema path depends on). Measured −57.1% vs minified JSON on a 20×5 table through `read agent --skip-guide`. Adversarial round-trip + validation-equivalence tests cover it. (done 2026-06-11)
- [x] **patch-html structural confirmation (F16 ergonomics)** — on success, patch-html now prints where the fragment landed and the match count, e.g. `patch-html: appended into <tbody id="risk-rows"> (1 match for "#risk-rows")`. A mis-target shows `into <body>`, so agents catch placement errors without a `read human` round-trip. (done 2026-06-11)
- [x] **H1/H2 harness (composition-controlled run)** — H1 patch-html hops now hand agents pre-written fragment files (`_inputs/h1-supplied/hop02-risk-fragment.html`, `hop06-svg-fragment.html`) + the exact `--selector`/`--patch-action` command (both arms, for fairness); H2 free hops carry a ≤300-char data budget. Documented in the workflow header as a separate, composition-controlled run (NOT comparable to the 2026-06-11-H baseline). Re-run pending to produce new C-REVISION-TOKENS / C-SYNTH-WIN numbers. (changes done 2026-06-11)
- [x] **C-GUIDE-STABLE / C-SCAFFOLD — corpus guide drift** — the D-INJECT corpus mixed THREE guide versions (the static `include_bytes!(spec/agent-guide.md)` grew 7599 → 8127 → 8840 bytes across the F15 update), so the original "rebuild flow-01/02" plan would have made it worse (a 3rd version). Instead refreshed the static spec entries (`spec/agent-guide.md`, `spec/clan.md`) + their manifest sha256 in all 41 corpus snapshots to the current bytes (`refresh-corpus-guide.py`, deterministic, agent content untouched). Result: 1 guide hash across 41 files (C-GUIDE-STABLE PASS), scaffold intercept a=2973 ≤ 3000 (C-SCAFFOLD PASS, near bound), guide-skip delta spread 0. (done 2026-06-11)
- [x] **F17** `pack-html` now accepts `--agent`/`--action`/`--rationale`/`--pinned` for inline attribution — records `fields_changed: ["human/index.html"]` in the decision chain without a separate `patch-decision` call. Both flags required together; partial supply is an error. (done 2026-06-12)
- [x] **F18** Write-path guard + decision tree. `pack-html` now blocks when `structured:` data is present but the HTML view is unchanged ("data-only write the expensive way") — teaching error names `patch-data` and `--force`. `--force` bypasses with a one-liner note. When the view genuinely changes alongside data, a hint is emitted but the command proceeds. Decision tree added to `spec/agent-guide.md` (new section before Surgical Patching) and `agent-help`. (done 2026-06-12)
- [x] **(follow-up) C-CONF T20 / F2b** — root cause confirmed 2026-06-12: T20 was a false positive because `CLAN_NO_HINTS=''` (empty) was treated as disabled by `is_none()`. Fixed Rust check to `map_or(true, |v| v.is_empty())` so empty = hints on. F2b implemented in the same commit; T20 now passes for the right reason.

### Heavy-benchmark findings F16 (research/15 heavy pass, 2026-06-11)

- [x] **F16** `patch-html` orphaned a risk-table `<tr>` outside its table in 4/5 H1-H reps. **Root cause was NOT lol_html** (`el.append()` correctly inserts before the end tag). The real causes: (1) `patch-html` had **no `--selector` flag** — the target had to be hand-authored as `patch_selector:` frontmatter, which agents omit; (2) a missing `patch_selector` **silently defaulted to `body`**, dumping the fragment at the end of `<body>` (outside the table); (3) a frontmatter-less fragment passed to `patch-html` was misread as a **full-html replacement** that wiped the entire view down to the fragment. **Fix (2026-06-11):** added `--selector` and `--patch-action` flags to `patch-html` (flags override frontmatter; a selector forces patch-html mode); `patch-html` now always patches and never full-replaces (`PatchTargeting.force_patch_mode`); the no-selector `body` fallback now warns loudly on stderr; `--patch-action` is validated against the five legal verbs. SDK regression tests (`patch_html_cli_selector_targets_element_without_frontmatter`, `patch_html_force_mode_never_full_replaces`, `patch_html_cli_selector_overrides_frontmatter`) + conformance **T24**. agent-help updated to teach `--selector`/`--patch-action` and the `--action` vs `--patch-action` distinction. (done 2026-06-11)
- [x] **(F16 sidecar)** conformance **T12** was latently red — its fixture patched the bare `clan create` placeholder view, which has no `<body>` for `patch_selector: "body"` to match. Fixed the fixture to seed a real body+table view via `pack-html` first. (done 2026-06-11)

### Lite-benchmark findings F13–F15 (research/15 lite pass, 2026-06-11)

- [x] **F13** `patch-data`/`patch-state` now accept an inline JSON string (leading `{`/`[`) and `--set key=value` scalars, not just a file path or stdin. (done 2026-06-11)
- [x] **F14** `patch-data --append <key>` (repeatable) concatenates onto an array key instead of RFC-7396 wholesale replace, reusing `merge.rs`'s `append` semantics; documented in agent-help. (done 2026-06-11)
- [x] **F15 (provenance — priority)** Attribution required by default on `patch-data`, `patch-html`, and `patch-asset`: `--agent` + `--action` (optional `--rationale`/`--pinned`); teaching error names the flags + `--no-decision` opt-out; `fields_changed` computed exactly from merge-patch keys; agent-help + agent-guide + spec/clan.md updated. **`patch-state` deliberately excluded** — it is an agent's private scratchpad, so mandating a shared-chain decision per write would only add noise (confirmed with user 2026-06-11). (done 2026-06-11)
- [x] **F2b** staleness is too coarse: after a data-update, check changed keys against `{{key}}` bindings in `human/index.html` — all bound → view NOT stale, no hint; some unbound → hint names the orphaned keys ("`x` not reflected in view — patch-html or pack-html"). Kills the false hint 4 H1 receipts flagged. (done 2026-06-12)
