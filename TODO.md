...
- [ ] **F17** `pack-html` does not accept `--agent`/`--action` flags — agents who need to attribute a view-pack step must use a separate `patch-decision` call as a workaround. Surfaced in B-META run 2026-06-11-D.
- [ ] **F18** Unguided agents do not discover the `patch-data` / `pack-html` split — they default to `pack-html` for all writes (including structured-data fields), which risks clobbering concurrent edits and wastes tokens on full-html re-renders. The agent-help / agent-guide should include a short "write path decision tree": data fields → `patch-data`; view layer only → `pack-html`; decision only → `patch-decision`. Surfaced in B-UNGUIDED run 2026-06-11-D (hop-3 discovery=9; all three hops used pack-html exclusively).
